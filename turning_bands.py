...
# point variance
print("sigma = ", np.sqrt(np.var(z.ravel())))

# resulted variogram
coarse_param = 1
data = np.array([x[::coarse_param, ::coarse_param].ravel(), y[::coarse_param, ::coarse_param].ravel(), z.ravel()]).transpose()
pwdist = pairwise( data )
tol = dx / 2.0
lags = np.arange(0.7 * dx, np.sqrt(2.0) * max_x / 2.0, 2.0 * tol)
